...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:scott12345@localhost:5432/fyyurapp'
...
def venues():
    ...
    for venue in venues:
        ...
        if venue_obj is not None:
            v_obj.append(tmp_obj)
            venue_obj['venues'] = v_obj
...
def search_venues():
    ...
    for venue in venues:
        all_shows = list(filter(lambda show: show.start_time.strftime(
            '%Y-%m-%d %H:%S:%M') >= cur_time, venue.artists))
        obj = {
            'id':venue.id,
            'name': venue.name,
            'num_upcoming_shows':len(all_shows) 
...
def show_venue(venue_id):
    ...
    old_shows = list(filter(lambda show: show.start_time.strftime('%Y-%m-%d %H:%S:%M') <=
                                cur_time, venue_query.artists))
    new_shows = list(filter(lambda shw: shw.start_time.strftime('%Y-%m-%d %H:%S:%M') >=
                                cur_time, venue_query.artists))
...
def search_artists():
    ...
    for artist in artist_query:
        all_shows = list(filter(lambda show: show.start_time.strftime(
            '%Y-%m-%d %H:%S:%M') >= cur_time, artist.venues))
        obj = {
            'id':artist.id,
            'name': artist.name,
            'num_upcoming_shows':len(all_shows) 
